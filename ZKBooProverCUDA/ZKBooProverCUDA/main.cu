...
// // #define WARP_SIZE			   32
// // #define GENKEY_WARP_PER_BLOCK   1
// // #define PATH_NUM			       3      // 多方安全计算分裂的个数
// // #define NUM_ROUNDS			   136    // 协议进行的次数
// // 

static const int GENKEY_THREAD_PER_BLOCK = WARP_SIZE * GENKEY_WARP_PER_BLOCK;            // 32个 threads
...
__global__ void generateKey(unsigned long long seed, int bytes) {
	...
	if (rgid < NUM_ROUNDS) 
	{
		...
		// 	shares[rgid][gpid][i] = dXorShift32Rand(&state);    // 这个就是x
	}
}
...
int main() {
	...
	// fgets(in, sizeof(in), stdin);       // 获取input x

	int len = strlen(in) - 1;   // 计算出x的长度
	...
	// // 、
	// // input是声明为__device__的变量。   (global 变量)
	cudaMemcpyToSymbol(input, in, INPUT_MAX_SIZE);
	// cudaMemcpyToSymbol(input, in, INPUT_MAX_SIZE);    // 将 in 拷贝到 input (GPU)中

...
uint32_t hhA[5] = { 0x67452301, 0xEFCDAB89, 0x98BADCFE, 0x10325476,0xC3D2E1F0 };
...
__host__ void hMpcXor(uint32_t *x, uint32_t *y, uint32_t *z)
{
	z[0] = x[0] ^ y[0];
	z[1] = x[1] ^ y[1];
	z[2] = x[2] ^ y[2];
...
__host__ void hMpcAnd(uint32_t *x, uint32_t *y, uint32_t *z, unsigned char (*randomness)[RANDTAPE_SIZE], int* randCount, View* views, int* countY)
{
	uint32_t r[3] = { hgetRandom32(randomness[0], *randCount), hgetRandom32(randomness[1], *randCount), hgetRandom32(randomness[2], *randCount)};
	*randCount += 4;
	uint32_t t[3] = { 0 };
...
__host__ void hMpcNegate(uint32_t *x, uint32_t *z)
{
	z[0] = ~x[0];
	z[1] = ~x[1];
	z[2] = ~x[2];
...
__host__ void hMpcAdd(uint32_t *x, uint32_t *y, uint32_t *z, unsigned char (*randomness)[RANDTAPE_SIZE], int* randCount, View* views, int* countY)
{
	uint32_t c[3] = { 0 };
	uint32_t r[3] = { hgetRandom32(randomness[0], *randCount), hgetRandom32(randomness[1], *randCount), hgetRandom32(randomness[2], *randCount)};
	*randCount += 4;
...
__host__ void hMpcAddK(uint32_t *x, uint32_t y, uint32_t *z, unsigned char (*randomness)[RANDTAPE_SIZE], int* randCount, View* views, int* countY)
{
	uint32_t c[3] = { 0 };
	uint32_t r[3] = { hgetRandom32(randomness[0], *randCount), hgetRandom32(randomness[1], *randCount), hgetRandom32(randomness[2], *randCount)};
	*randCount += 4;
...
__host__ void hMpcRightRotateOneCore(uint32_t *x, int i, uint32_t *z)
{
	z[0] = RIGHTROTATE(x[0], i);
	z[1] = RIGHTROTATE(x[1], i);
	z[2] = RIGHTROTATE(x[2], i);
...
__host__ void hMpcLeftRotate(uint32_t *x, int i, uint32_t *z)
{
	z[0] = LEFTROTATE(x[0], i);
	z[1] = LEFTROTATE(x[1], i);
	z[2] = LEFTROTATE(x[2], i);
...
__host__ void hMpcRightShift(uint32_t *x, int i, uint32_t *z)
{
	z[0] = x[0] >> i;
	z[1] = x[1] >> i;
	z[2] = x[2] >> i;
...
__host__ void hMpcMAJ(uint32_t *a, uint32_t *b, uint32_t *c, uint32_t *z, unsigned char (*randomness)[RANDTAPE_SIZE], int* randCount, View* views, int* countY)
{
	uint32_t t0[3];
	uint32_t t1[3];
	...
	hMpcXor(a, b, t0);
	hMpcXor(a, c, t1);
	hMpcAnd(t0, t1, z, randomness, randCount, views, countY);
	hMpcXor(z, a, z);
}
...
__host__ void hMpcSha1(unsigned char (*results)[32], unsigned char* input[3], int numBits, unsigned char (*randomness)[RANDTAPE_SIZE], View views[3], int* countY)
{
	...
	for(int i=0; i<3 ; i++)
	{
		memcpy(chunks[i], input[i], chars);
		chunks[i][chars] = 0x80;

		memset(chunks[i] + chars + 1, 61 - chars, 0);
		chunks[i][62] = numBits >> 8;
		chunks[i][63] = numBits;
	...
	for (int j = 16; j < 80; j++) {
		hMpcXor(w[j-3], w[j-8], temp);
		hMpcXor(temp, w[j-14], temp);
		hMpcXor(temp, w[j-16], temp);
		hMpcLeftRotate(temp,1,w[j]);
	}

	uint32_t a[3] = { hhA[0],hhA[0],hhA[0] };
	uint32_t b[3] = { hhA[1],hhA[1],hhA[1] };
	uint32_t c[3] = { hhA[2],hhA[2],hhA[2] };
	uint32_t d[3] = { hhA[3],hhA[3],hhA[3] };
	uint32_t e[3] = { hhA[4],hhA[4],hhA[4] };
	uint32_t f[3];
	uint32_t k;
	...
	for (int i = 0; i < 80; i++) {
		if(i <= 19) {
			//f = d ^ (b & (c ^ d))
			hMpcXor(c,d,f);
			hMpcAnd(b, f, f, randomness, &randCount, views, countY);
			hMpcXor(d,f,f);
			k = 0x5A827999;
		}
		else if(i <= 39) {
			...
			hMpcMAJ(b,c,d,f,randomness, &randCount, views, countY);
		...
		//temp = (a leftrotate 5) + f + e + k + w[i]
		hMpcLeftRotate(a,5,temp);
		hMpcAdd(f,temp,temp,randomness, &randCount, views, countY);
		hMpcAdd(e,temp,temp,randomness, &randCount, views, countY);
		hMpcAddK(temp,k,temp,randomness, &randCount, views, countY);
		hMpcAdd(w[i],temp,temp,randomness, &randCount, views, countY);

		memcpy(e, d, sizeof(uint32_t) * 3);
		memcpy(d, c, sizeof(uint32_t) * 3);
		hMpcLeftRotate(b,30,c);
		memcpy(b, a, sizeof(uint32_t) * 3);
		memcpy(a, temp, sizeof(uint32_t) * 3);
	}

	uint32_t hHa[5][3] = { { hhA[0],hhA[0],hhA[0]  }, { hhA[1],hhA[1],hhA[1] }, { hhA[2],hhA[2],hhA[2] }, { hhA[3],hhA[3],hhA[3] },
			{ hhA[4],hhA[4],hhA[4] }};

	hMpcAdd(hHa[0], a, hHa[0], randomness, &randCount, views, countY);
	hMpcAdd(hHa[1], b, hHa[1], randomness, &randCount, views, countY);
	hMpcAdd(hHa[2], c, hHa[2], randomness, &randCount, views, countY);
	hMpcAdd(hHa[3], d, hHa[3], randomness, &randCount, views, countY);
	hMpcAdd(hHa[4], e, hHa[4], randomness, &randCount, views, countY);

	for (int i = 0; i < 5; i++) {
		hMpcRightShift(hHa[i], 24, t0);
		results[0][i * 4] = t0[0];
		results[1][i * 4] = t0[1];
		results[2][i * 4] = t0[2];
		hMpcRightShift(hHa[i], 16, t0);
		results[0][i * 4 + 1] = t0[0];
		results[1][i * 4 + 1] = t0[1];
		results[2][i * 4 + 1] = t0[2];
		hMpcRightShift(hHa[i], 8, t0);
		results[0][i * 4 + 2] = t0[0];
		results[1][i * 4 + 2] = t0[1];
		results[2][i * 4 + 2] = t0[2];
